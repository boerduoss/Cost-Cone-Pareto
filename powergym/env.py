...
class Env(gym.Env):
    ...
    def __init__(self, folder_path, info, dss_act=False):
        ...
        self.load_profile = LoadProfile(\
                 ...
                 run_token = info['run_token'] if 'run_token' in info else None)
    ...
    class MyReward:
        ...
        def composite_reward(self, cd, rd, soc, dis, full=True, record_node=False):
            ...
            # the reward function don't consider the voltage violation
            p = self.powerloss_reward()
            v, vio_nodes = self.voltage_reward(record_node)
            t = self.ctrl_reward(cd, rd, soc, dis)
            summ = p + t
    ...
    def plot_graph(self, node_bound='minimum', 
        ...
        nx.draw_networkx_edges(graph, pos, loc_lines, edge_color='k', width=3, label='lines', ax=ax)
        nx.draw_networkx_edges(graph, pos, loc_trans, edge_color='r', width=3, label='transformers', ax=ax)
        if show_voltages:
            nx.draw_networkx_nodes(graph, pos, nodelist=nodes, node_color=voltages, vmin=vmin, vmax=vmax, cmap=cmap, node_size=node_size, ax=ax)
            sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin=vmin, vmax=vmax))
            sm.set_array([])
            fig.colorbar(sm, ax=ax)
        else:
            nx.draw_networkx_nodes(graph, pos, nodelist=nodes, node_color=np.ones(len(voltages)), vmin=vmin, vmax=vmax, cmap=cmap, node_size=node_size, ax=ax)

        if show_node_labels:
            node_labels = {node:node for node in pos}
            nx.draw_networkx_labels(graph, pos, labels= node_labels, font_size=15, ax=ax)

        # show source bus
        loc={self.source_bus:(pos[self.source_bus][0]+shift, pos[self.source_bus][1]-shift)}
        nx.draw_networkx_labels(graph, loc, labels={self.source_bus:'src'}, font_size=15, ax=ax)
        ...
        if show_actions:
            ax.text(text_loc_x, text_loc_y, s='t='+str(self.t)+' Action: '+ self.str_action,
                    fontsize=18)
        elif show_voltages:
            ax.text(text_loc_x, text_loc_y, s='t='+str(self.t), fontsize=18)
